/*
Class for defining point patterns under periodic boundary conditions
	Supports Cartesian, Relative, and Lattice-based coordinate systems
	Defines basic structures and types for manipulating simulation box and particles contained within it
Also defines methods for iterating through all particles contained within simulation
Assumed that fundamental cells of each patterns are Bravais lattices (described by set of primitive lattice vectors)
Reading/Writing to ConfigPack function declarations at the very bottom
...
// Configuration & SpherePacking are derived from this.
...
protected:
	...
	// convert dimensional index to the 1-dimension cell index
	Index ConvertIndex(const signed long * index) const {
	...
	//get the index of the cell that a should be placed in, the LatticeCoordinates of particle must be in box
	Index GetIndex(const size_t a) const  {
...
public:
	...
	void GetVicinityLattice(double Rc) const
	{
		...
		if (this->SortedVicinityList) {
			std::sort(this->VicinityLattices.begin() + PrevVicinityLatticeNum, this->VicinityLattices.end(), [](const VicinityLatticesMember & left, const VicinityLatticesMember & right) -> bool
			{
				return left.CartesianCoordinates.Modulus2() < right.CartesianCoordinates.Modulus2();
			});
		}
	...
	// PeriodicCellList< of anything > can be converted to PeriodicCellList<Empty>
	...
	GeometryVector displacement_periodic_relative (int i, const GeometryVector & r0_Relative) const {
		if (i < this->NumParticle()) {
			...
			for (int k = 0; k < this->Dimension; k++) {
				if (Displacement.x[k] < -0.5) {
					Displacement.x[k] += 1.0;

				} else if (Displacement.x[k] > 0.5) {
					Displacement.x[k] -= 1.0;
				}
			}
