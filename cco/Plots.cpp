...
void PlotPacking2D(std::string prefix, const SpherePacking & List, const char * sphereColor, const std::string & remark)
{
	const SpherePacking * list= & List;
	const char * color[] = {"k-", sphereColor};
#ifdef USE_MATHGL
	...
	if(list->GetDimension()==2)
	{
		...
		//gr.SetOrigin(0, 0);
		gr.Puts(mglPoint(0.5*(max+min), min+1.1*(max-min)), remark.c_str());
		//gr.Puts(mglPoint(0, 0), remark.c_str());
		...
		for(int i=0; i<list->NumParticle(); i++)
		{
			...
			GeometryVector now=list->GetCartesianCoordinates(i);
			::PlotDisk2D(gr, now.x[0], now.x[1], r, color[1]);
...
{
	PlotPacking2D(prefix, SpherePacking(List, r), "k", remark);
}
