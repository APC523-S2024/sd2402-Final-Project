...
void GetLatticeTerm(Configuration & crystal, std::vector<LatticeTerm> & result, double Rc)
{
	...
	for (size_t i = 0; i < crystal.NumParticle(); i++) {
		crystal.IterateThroughNeighbors(crystal.GetRelativeCoordinates(i), Rc, [&distance2s] (const GeometryVector & shift, const GeometryVector & LatticeShift, const signed long * PeriodicShift, const size_t SourceAtom) -> void
		{
